...
class generalProcessor(DataProcessor1):
    ...
    def _create_examples(self, path, docs, set_type):
        ...
        use_case = '_'.join(path.split(os.sep)[-2:])
        val = ContrastiveClassificationDataset(
            path + f"/{use_case}-gs.json.gz", dataset_type='test', tokenizer='roberta-base', dataset=use_case
        )
