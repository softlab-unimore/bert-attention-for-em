...
class WordMask(nn.Module):
    ...
    def forward(self, model, inputs, pred, flag, thr=None):
        ...
        if flag == 'train':
            ...
            x_prime = torch.cat((
                r_s.new_ones(r_s.shape[0], 1, r_s.shape[2]),  # <s> inalterato
                r_s[:, :self.sent1_len, :],
                r_s.new_ones(r_s.shape[0], 1, r_s.shape[2]),  # </s> inalterato
                r_s.new_ones(r_s.shape[0], 1, r_s.shape[2]),  # </s> inalterato
                r_s[:, self.sent1_len:, :],
                r_s.new_ones(r_s.shape[0], x.shape[1] - r_s.shape[1] - 3, r_s.shape[2])),
                dim=1) * x

            attention_masks_r = torch.cat((r_s.new_ones(r_s.shape[0], 1),
                                           r_s[:, :self.sent1_len].squeeze(-1),
                                           r_s.new_ones(r_s.shape[0], 2),
                                           r_s[:, self.sent1_len:].squeeze(-1),
                                           r_s.new_ones(r_s.shape[0], x.shape[1] - r_s.shape[1] - 3))
                                          , dim=1) * attention_masks_r
            input_ids_r = torch.cat((r_s.new_ones(r_s.shape[0], 1),
                                     r_s[:, :self.sent1_len].squeeze(-1),
                                     r_s.new_ones(r_s.shape[0], 2),
                                     r_s[:, self.sent1_len:].squeeze(-1),
                                     r_s.new_ones(r_s.shape[0], x.shape[1] - r_s.shape[1] - 3))
...
def interpret(args, model, inputs, pred, thr=None):
    ...
    sent1_len = sep_idx[0] - 1
    sent2_len = sep_idx[2] - sep_idx[1] - 2
