...
from transformers import RobertaForSequenceClassification, RobertaConfig, RobertaTokenizer, AutoTokenizer

from utils_glue_supcon import (convert_examples_to_features,
                              output_modes, processors)
from core.modeling.supcon import ContrastiveClassifierModel
from group_mask_supcon import interpret, mostImpWord
...
def createOrdImp(args, model, tokenizer, eval_dataset, listInfo, listAttrName):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for btxt in batch[0].data[0]:
            btxt = int(btxt)
            if tokenizer.ids_to_tokens[btxt] != '[PAD]' and tokenizer.ids_to_tokens[btxt] != '[SEP]' and \
                    tokenizer.ids_to_tokens[btxt] != '[CLS]':
...
def test_explain(args, model, tokenizer, eval_dataset, listInfo, listAttrName):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            inputs = {'input_ids': batch[0],
                      'attention_mask': batch[1],
                      'input_ids_right': batch[2],
                      'attention_mask_right': batch[3],
                      'labels': batch[5]}
            outputs = model(**inputs)
            tmp_eval_loss, logits = outputs[:2]
...
def load_and_cache_examples(args, task, tokenizer, type):
    ...
    fake_token_type_ids = torch.zeros_like(examples['input_ids'])
    dataset = TensorDataset(examples['input_ids'], examples['attention_mask'], examples['input_ids_right'],
                            examples['attention_mask_right'], fake_token_type_ids,
                            examples['labels'], examples['evidence'])
...
def main():
    ...
    config.vocab_size = config.vocab_size + 2  # Add the special tokens [COL] and [VAL]

    # Load the model
    use_case = '_'.join(args.output_dir.split(os.sep)[-2:])
    model_path = args.output_dir + f"/{use_case}.bin"
    tuned_model = ContrastiveClassifierModel(
        checkpoint_path=model_path,
        len_tokenizer=config.vocab_size,
        model=args.model_type,
        frozen=True,
        device=args.n_gpu
    )
    # from core.modeling.supcon import BaseEncoder
    # base_encoder = BaseEncoder(len_tokenizer=config.vocab_size, model=args.model_type)
    # base_encoder.transformer = tuned_model.encoder.transformer
    # tuned_model.encoder = base_encoder
    # Map SupCon on RobertaForSequence classification
    ...
    tuned_model.to(args.device)

    # Load the tokenizer
    tokenizer = AutoTokenizer.from_pretrained('roberta-base', additional_special_tokens=('[COL]', '[VAL]'),
                                              do_lower_case=args.do_lower_case)
    # tokenizer = tokenizer_class.from_pretrained(args.output_dir, additional_special_tokens=('[COL]', '[VAL]'),
    #                                             do_lower_case=args.do_lower_case)
    ...
    post_acc = test_explain(args, tuned_model, tokenizer, test_dataset, listInfo, listAttr)
